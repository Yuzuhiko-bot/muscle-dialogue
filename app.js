...
以下のルールはアプリの仕様（UIや安全機能）に関わるため、いかなる理論よりも優先して厳守すること。
 ...
 9. **ターゲット回数設定の厳格化**:
    ...
    ※ただし、ルール16の漸進性過負荷の適用により回数が増加する場合は、この回数上限を一時的に突破してもよい。あるいは、上限に達している場合は重量を1ステップ上げて回数を下限に戻すこと。
 ...
 16. **漸進性過負荷（Progressive Overload）の徹底（筋肥大目的の場合）**:
    ...
    b) RPE 8〜9（ほぼ限界）→ 原則として重量は維持し、回数を1〜2回増やす。ただし、前回の回数が既にターゲット回数の上限（主役なら8回、脇役なら15回）に達している場合は、重量を1ステップ上げ、回数を下限（主役なら6回）にリセットすること。
    ...
    ※【重要・絶対遵守事項】いかなるルール（ターゲット回数の上限など）の制約下であっても、「前回の挙上重量・回数（ボリューム）」を下回る提案をすることは絶対悪であると認識し、絶対に避けること。「前回と同じ」をデフォルト出力にすることも禁止。常に「何を1つ上げるか」を note に明記すること。
     【本日の名言】：${randomQuote}
