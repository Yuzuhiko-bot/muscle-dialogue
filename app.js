...
以下のルールはアプリの仕様（UIや安全機能）に関わるため、いかなる理論よりも優先して厳守すること。
 ...
 3. **マンネリ打破**: 直近履歴で停滞期（数週間同じ内容）の兆候がある場合は、別種目で刺激を変えるか、理論（負荷帯の変更など）を適用すること。
 4. **怪我の配慮と自由要望**: 指定された痛み部位の種目は完全除外。自由要望がある場合は全てのルールより最優先する。
 5. **絶対除外リストの遵守（最優先）**: ユーザー情報の「絶対除外リスト」に掲載されている部位は、他のいかなる理論よりも優先して、本日のメニューから「完全除外」すること。
 6. **最低重量の厳守**: 各種目に【最低重量: Xkg】が設定されている場合、提案する weight_kg は絶対にその数値以上にすること。
 7. **トーン＆マナー**: 「礼儀正しく、シンプルで熱いトーン」。長々とした解説は避け、テンポ良くまとめること。最後のメッセージには必ず以下の名言を組み込むこと。
    【本日の名言】：${randomQuote}
 8. **マッスル・ルール（ユーザー個別ルール）の厳守**:
    以下のユーザー固有のルールを「絶対除外リスト」と同等の最優先事項として遵守すること。
${(state.userRules || []).map((r, i) => `    - ${i + 1}. ${r}`).join('\n') || '    - (特になし)'}
`;

  const HYPERTROPHY_RULES = `
## 🔥 【筋肥大・筋力アップ専用ルール】
 1. **ターゲット回数・インターバル設定の厳格化**:
    - 主役（High）部位：3〜6回（筋力向上・神経系）または 6〜8回（高重量肥大）。インターバル3〜5分。
    - 脇役（Low）部位：10〜15回（代謝的ストレス・パンプ）。インターバル1.5〜2分。
    ※ただし、漸進性過負荷の適用により回数が増加する場合は、この回数上限を一時的に突破してもよい。あるいは、上限に達している場合は重量を1ステップ上げて回数を下限に戻すこと。
 2. **部位別フォーカス型の波状期分け（DUP）の自動適用**: 
    - 同一部位を週2回以上鍛える分割法の場合、直近履歴から日によって「主役（High）」と「脇役（Low）」を交代させること。主役は1種目目に配置。
    ※「その部位を最後に主役（High）で追い込んだ日」がより遠い方を本日の主役として優先すること。
 3. **サブ部位の網羅的アプローチ（偏りの防止）**: 
    - 特定のサブ部位（例：胸の上部ばかり等）に種目が偏ることを絶対に防ぐこと。週複数回鍛える場合は、ターゲットとなるサブ部位を意図的にズラして立体的な筋肉をデザインすること。
 4. **重点部位（メイン筋肉）の優先順位**: ユーザー情報の「重点部位」は常に「主役（High）」候補として最優先に検討すること。
 5. **総ボリュームの最適化**: 週頻度が高い（週5〜6回）場合、1セッションあたりの合計セット数を増やしすぎないこと（目安：5〜7種目、計15〜20セット以内）。
 6. **漸進性過負荷（Progressive Overload）の徹底**:
    - 筋肥大には「前回のセッションを1%でも超える刺激」が不可欠である。直近の履歴で同一種目の実績とRPEがある場合、以下の優先順位で必ず負荷を漸増させること。
      ...
      b) RPE 8〜9（ほぼ限界）→ 原則として重量は維持し、回数を1〜2回増やす。ただし上限回数に達しているなら重量を上げ回数をリセット。
      c) RPE 10（完全な限界）→ 重量・回数を維持しセット数追加か、テンポコントロールで質を変える。
    ※【重要・絶対遵守事項】いかなるルールの制約下であっても、「前回の挙上重量・回数（ボリューム）」を下回る提案をすることは絶対悪であると認識し、絶対に避けること。「前回と同じ」をデフォルト出力にすることも禁止。常に「何を1つ上げるか」を note に明記すること。
`;

  const DIET_RULES = `
## 🏃 【ダイエット・健康維持専用ルール】
 1. **ターゲット回数・インターバル設定**:
    - 15〜20回（関節への負担軽減と代謝アップ）。
    - インターバルは2〜5分（多関節種目は十分な回復を待つため長く取る）。
 2. **疲労管理と負荷設定（オーバートレーニング防止）**:
    - カロリー制限中は回復力が低下するため、限界（RIR 0）まで追い込まず、RIR 1〜3（余力を残す）で止めること。
 3. **ボリュームの最適化と過負荷の免除**:
    - 無理な過負荷（漸進性過負荷）を強制しないこと。
    - ダイエット中はセット数を維持するか意図的に削減してもよい。前回よりボリューム（セット数や回数）が落ちても、重量（扱う重さ）が維持されていれば、筋肉量は保護されるため「良し」とする。
    ※前回のボリュームを下回っても構わないので、怪我を防ぎ強度の維持（重量を落とさないこと）を最優先すること。
`;

  let selectedGoalRules = (p.goal === "ダイエット" || p.goal === "健康維持") ? DIET_RULES : HYPERTROPHY_RULES;
